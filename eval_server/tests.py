...
def outer_test_blinding(key: RSA.RsaKey, C: int):  # Challenge #1
    ...
    def test_blinding(s: str):
        ...
        C_times_s: bytes = long_to_bytes((C * pow(s, key.e, key.n)) % key.n)
        if check_padding_private_key(C_times_s, key):
            return (
                True,
