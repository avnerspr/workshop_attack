...
def get_public() -> tuple[int, int]:
    """
    Reads a public RSA key from the file "public_key.rsa" and returns the modulus (n) and public exponent (e).

    This function opens the public key file, imports it using the `Crypto.PublicKey.RSA` library,
    and extracts the modulus and exponent used for the encryption.

    Returns:
        tuple[int, int]: A tuple containing the modulus n and public exponent e of the RSA key.
    """
...
def get_cipher() -> int:
    """
    Generates a ciphertext by encrypting a predefined plaintext message using the RSA public key
    found in the "public_key.rsa" file. The ciphertext is returned as an integer.

    This function demonstrates how to encrypt a plaintext message using the RSA encryption scheme,
    which will later be attacked using the Bleichenbacher attack.

    Returns:
        int: The ciphertext, represented as an integer.
    """
...
if __name__ == "__main__":
    ...
    attacker = MultiServerAttacker(
        n, e, get_cipher(), HOSTS, PORTS, my_args.random, my_args.verbose
    )
